...
interface NewCompetitorTableDrawerProps {
    isOpen: boolean
    onClose: () => void
    onSubmit: (title: string) => Promise<void>
}

export function NewCompetitorTableDrawer({
    isOpen,
    onClose,
    onSubmit
}: NewCompetitorTableDrawerProps) {
        ...
        <>
            ...
            <div className="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border border-[#27272a] bg-[#141414] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                <div className="flex h-12 items-center justify-between border-b border-[#27272a] px-4">
                    <span className="text-sm font-semibold text-white">Buat Tabel Bidang Kompetitor</span>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className="p-4 flex flex-col gap-4">
                    <div>
                        <label className="block text-xs text-zinc-400 mb-1.5">Nama Bidang / Kategori</label>
                        <input
                            type="text"
                            value={title}
                            onChange={(e) => setTitle(e.target.value)}
                            placeholder="Misal: Furniture, Keramik..."
