...
export function ContentSidebar({
    ...
    const taskTable = tables.find(t => t.type === 'tasks')
                ...
                {editingId === t.id ? (
                    ...
                    <>
                        ...
                        >
                            ...
                            <span className="flex-1 truncate">{t.title || `Sprint ${t.sprint_number}`}</span>
                        </button>
                        <div
                            role="button"
                            tabIndex={0}
    ...
    return (
        <aside className="flex h-full w-[220px] flex-col shrink-0 border-r border-[#27272a] bg-[#141414] overflow-y-auto scrollbar-thin">
            ...
            <div className="p-4 flex flex-col gap-1">
                ...
                {/* Tasks section */}
                {taskTable && renderRow(taskTable)}

                {/* Sprints section */}
                <div className="mt-4 mb-1 px-2">
                    <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">Sprint</span>
                </div>

                {sprintTables.length === 0 ? (
                    <p className="px-3 py-2 text-xs text-zinc-600 italic">Belum ada sprint</p>
                ) : (
                    sprintTables.map(renderRow)
                )}
            </div>
        </aside>
    )
